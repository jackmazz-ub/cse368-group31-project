...
class Agent:
    ...
    def remember(self, state, action, reward, next_state, game_over):
        self.memory.append((state, action, reward, next_state, game_over)) #pop left is MAX_MEMORY
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 200) < self.eplison:
            ...
            perdiction = self.model(state0)
            move = torch.argmax(perdiction).item()
            final_move[move] = 1
        return final_move
...
def train():
    ...
    while True:
        ...
        if done: #game over
            # train long memory - replay/experince memory - train on previous moves done
            # plot results
            game.reset()
            agent.n_games += 1
